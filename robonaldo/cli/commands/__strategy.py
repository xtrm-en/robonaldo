from ..command import Command, CommandContext, register, by_name, commands
...
class Strategy(Command):
            ...
            if ctx.args[0] == "toggle":
                ...
                if strat_id == "all":
                    for strat in StrategyManager().strategies.values():
                        StrategyManager().set_enabled(
                            strat, not StrategyManager().is_enabled(strat)
                        )
                    return (True, "")
    ...
    def usages(self) -> List[str]:
        return [
            "%NAME% list",
            "%NAME% <toggle/enable/disable> <strat_id>",
            "%NAME% <toggle/enable/disable> all",
        ]
